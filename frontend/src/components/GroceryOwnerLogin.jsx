...
const OwnerLogin = () => {
    ...
    const { loggedin, setLoggedin, email, setEmail, setUserType } = useContext(LoginContext);
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const response = await fetch(`${import.meta.env.VITE_API_URL}groceryowners`);
                if (!response.ok) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isValidUser) {
                ...
                setUserType('groceryOwner')
                console.log("Account verified successfully !!!");
                setSuccess("Login successful! ");
                setTimeout(() => navigate('/GroceryOwner/Dashboard'), 1300);
            } else {
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-center space-x-2 mt-2">
                        <NavLink
                            ...
                            to="/groceryowner/register"
                        >
                            Sign up as Grocery Store Owner
                        </NavLink>
                    </div>
                </form>
            </div>
